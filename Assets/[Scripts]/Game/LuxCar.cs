...
using DG.Tweening;
public class LuxCar : MonoBehaviour
{
    ...
    public int carMoneyFive;
    public int carMoneySix;
    ...
    void Update()
    {
        ...
        CarToMoney(4, carMoneyFive);
        CarToMoney(5, carMoneySix);
    }
    public void CarToMoney(int _moneyCarPart,int _currentMoneyPart)
    {
        if (carLevel == _currentMoneyPart)
        {
            ...
            Instantiate(Resources.Load("particles/Cloud"), carMoneyList[_moneyCarPart].transform.position, Quaternion.identity);
        }    
    }
